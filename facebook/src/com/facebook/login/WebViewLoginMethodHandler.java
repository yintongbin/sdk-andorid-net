...
class WebViewLoginMethodHandler extends LoginMethodHandler {
	...
	boolean tryAuthorize(final LoginClient.Request request) {
		...
		String previousTokenString = previousToken != null ? previousToken
				.getToken() : null;
		...
		WebDialog.Builder builder = new AuthDialogBuilder(fragmentActivity,
				request.getApplicationId(), parameters).setE2E(e2e)
		...
		if (values != null) {
			...
			try {
				...
				outcome = LoginClient.Result.createTokenResult(
						loginClient.getPendingRequest(), token);
				...
				CookieSyncManager syncManager = CookieSyncManager
						.createInstance(loginClient.getActivity());
				syncManager.sync();
			...
			if (error instanceof FacebookOperationCanceledException) {
				outcome = LoginClient.Result.createCancelResult(
						loginClient.getPendingRequest(),
						"User canceled log in.");
			} else {
				// Something went wrong, don't log a completion event since it
				// will skew timing
				// results.
				e2e = null;
				...
				if (error instanceof FacebookServiceException) {
					FacebookRequestError requestError = ((FacebookServiceException) error)
							.getRequestError();
					errorCode = String.format(Locale.ROOT, "%d",
							requestError.getErrorCode());
					errorMessage = requestError.toString();
				}
				outcome = LoginClient.Result.createErrorResult(
						loginClient.getPendingRequest(), null, errorMessage,
						errorCode);
			}
		}
